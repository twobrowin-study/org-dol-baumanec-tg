...
func (app *App) SetDoctorDone(chatId string, user *User, doctorId string) {
	...
	var doctorFun bool
	err = app.DbConnection.QueryRow(context.Background(), "select title, doctor_type, is_fun from doctor where id=$1", doctorId).Scan(&doctorTitle, &doctorType, &doctorFun)
	errExx("SetDoctorDone doctor", err)
	app.send(chatId, "Отмечено ✅ " + doctorTitle)

	if user.Sex == "male" && doctorFun {
